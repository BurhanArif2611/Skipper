...
class NotificationController extends GetxController implements GetxService {
  ...
  Future<void> getNotificationList(int offset,bool reload) async {
    /*if (offset == 1) {*/
   ...
   /* if(_notificationList == null || reload && offset == 1) {
    ...
    final prefs = await SharedPreferences.getInstance();
      Response response = await notificationRepo.getNotificationList(offset,prefs.getString(AppConstants.EmailID).toString());
      if (response.statusCode == 200) {
       // _offset=int.parse(response.body['offset']);

        _notificationList=  NotificationModel.fromJson(response.body);
       /* response.body['data'].forEach((notification) => _notificationList.add(NotificationModel.fromJson(notification)));
        _notificationList.sort((a, b) {
      ...
      */} else {
        _notificationList=  NotificationModel(id:0,data:[]);
        ApiChecker.checkApi(response);
      }
      update();
   /* }*/
